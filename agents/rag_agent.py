...
class RAGAgent:
    ...
    def retrieve_context(self, query: str, k: int = TOP_K_RESULTS) -> List[Dict[str, Any]]:
        """
    ...
    def synthesize_answer(self, question: str, context_docs: List[Dict[str, Any]]) -> str:
        ...
        prompt = self.synthesis_prompt_template.format(context=context, question=question)
    ...
    def answer_question(
        self, question: str, k: int = TOP_K_RESULTS, return_sources: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        result = {"answer": answer, "sources_used": len(context_docs)}

        if return_sources:
            result["sources"] = [
                {"source": doc["source"], "relevance_score": doc["score"]} for doc in context_docs
            ]
