...
def benchmark_workflow(use_parallel=True, use_cache=True, num_emails=3):
    ...
    workflow = SupportWorkflow(
        max_revisions=0,  # No retries for benchmark
...
def compare_configurations():
    ...
    print(f"{'Baseline (no optimization)':<30} {results_baseline['avg_time']:>10.2f}s    "
          f"{results_baseline['total_time']:>10.2f}s    1.00x")
    
    parallel_speedup = baseline_avg / results_parallel['avg_time'] if results_parallel['avg_time'] > 0 else 1.0
    print(f"{'+ Parallel processing':<30} {results_parallel['avg_time']:>10.2f}s    "
          f"{results_parallel['total_time']:>10.2f}s    {parallel_speedup:.2f}x")
    
    optimized_speedup = baseline_avg / results_optimized['avg_time'] if results_optimized['avg_time'] > 0 else 1.0
    print(f"{'+ Full optimization':<30} {results_optimized['avg_time']:>10.2f}s    "
    ...
    improvement = ((baseline_avg - results_optimized['avg_time']) / baseline_avg * 100) if baseline_avg > 0 else 0
    ...
    emails_per_hour_optimized = 3600 / results_optimized['avg_time'] if results_optimized['avg_time'] > 0 else 0
