        ...
        if use_fuzzy:
            for cached_key, entry in list(self.cache.items()):
                ...
                if entry['category'] == category:
                    if self._is_similar(email_subject + " " + email_body, 
...
if __name__ == "__main__":
    ...
    cache = ResponseCache(cache_file="test_cache.json", ttl_hours=1)
    
    # Store a response
    print("\n1. Storing response...")
    cache.set(
        email_subject="How do I reset my password?",
        email_body="I forgot my password and need to reset it",
        category="technical_support",
        response="Dear customer, to reset your password, please visit...",
        qa_score=8.5
    )
    
    # Retrieve exact match
    print("\n2. Retrieving exact match...")
    result = cache.get(
        email_subject="How do I reset my password?",
        email_body="I forgot my password and need to reset it",
        category="technical_support"
    ...
    if result:
        print(f"Found: {result['response'][:50]}...")
        print(f"QA Score: {result['qa_score']}")
    
    # Try fuzzy match
    print("\n3. Trying fuzzy match...")
    result = cache.get(
        email_subject="Reset password help",
        email_body="I can't remember my password and need help resetting",
        category="technical_support"
    )
    if result:
        print(f"Found (fuzzy): {result['response'][:50]}...")
    else:
